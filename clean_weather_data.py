...
def main():
    ...
    aggregated_weather_data = aggregated_weather_data.drop_duplicates(subset=['LOCAL_DATE', 'STATION_NAME', 'LATITUDE', 'LONGITUDE'], keep='last')

    # Gets all unique stations
    all_stations = aggregated_weather_data[['STATION_NAME', 'LATITUDE', 'LONGITUDE']].drop_duplicates(subset=['STATION_NAME', 'LATITUDE', 'LONGITUDE'], keep='last').values.tolist()
    ...
    # Filter outs the datapoints only from valid stations
    filled_in_aggregated_weather_data = filled_in_aggregated_weather_data[(filled_in_aggregated_weather_data["STATION_NAME"].isin(stations[:0])) & (filled_in_aggregated_weather_data["LATITUDE"].isin(stations[:1])) & (filled_in_aggregated_weather_data["LONGITUDE"].isin(stations[:2]))]
    ...
    for station in filtered_valid_stations:
        station_df = filled_in_aggregated_weather_data[(filled_in_aggregated_weather_data["STATION_NAME"] == station[0]) & (filled_in_aggregated_weather_data["LATITUDE"] == station[1]) & (filled_in_aggregated_weather_data["LONGITUDE"] == station[2])]
